...
class Parser(object):
        ...
        for i, path in enumerate(all_path):
            for j, (cmd, points) in enumerate(path):
                if cmd == 'l':
                    ...
                    new_all_path[i][j] = ('A', f'{rx + x} {ry + y} {angle_x_axis} {large_arc_flag} {sweep_flag} {ex + x} {ey + y}')
    ...
    def get_line(self, path_points, i):
        ...
        if cmd == 'L':
            ...
            return [Vector2(e_points, s_vec.y())]
        elif cmd == 'V':
            return [Vector2(s_vec.x(), e_points)]
        elif cmd == 'C':
            c1x,c1y, c2x,c2y, ex,ey = e_points.split(' ')
            return cubic_path(s_vec, Vector2(c1x,c1y), Vector2(c2x,c2y), Vector2(ex,ey))
        elif cmd == 'S':
            # Not Implemented
            return []
        elif cmd == 'Q':
            mx,my, ex,ey = e_points.split(' ')
            return quadratic_path(s_vec, Vector2(mx,my), Vector2(ex,ey))
        elif cmd == 'T':
            # Not Implemented
            return []
        elif cmd == 'A':
            rx, ry, angle_x_axis, large_arc_flag, sweep_flag, ex, ey = e_points.split(' ')
            return arc_path(s_vec, Vector2(ex, ey), float(rx), float(ry), float(angle_x_axis), int(large_arc_flag), int(sweep_flag))
        elif cmd in ['Z', 'z']:
    ...
    def add_rect(self, rect_element):
        ...
        lup   = Vector3(x         ,y          ,0)
        rdown = Vector3(x + width ,y + height ,0)
        rup   = Vector3(x + width ,y          ,0)
        ldown = Vector3(x         ,y + height, 0)
    ...
    def add_use(self, use_element):
        ...
        for path in paths:
            new_path = []
            for p in path:
                res_vec = Vector3(p.x() + vec.x(), p.y() + vec.y(), 0) * 0.1
                res_vec.xyz[1] *= -1
    ...
    def add_path(self, path_element):
        ...
        for path in paths:
            new_path = []
            for p in path:
                res_vec = Vector3(p.x() + vec.x(), (p.y() + vec.y()) * -1, 0) * 0.1
                # dis_from_origin = res_vec.distance(VEC3_ZERO)
                # if dis_from_origin < self.sdt:
                #     self.sdt = dis_from_origin
                #     self.top_left = res_vec
